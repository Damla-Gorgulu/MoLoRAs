#!/bin/bash
#
# v2.1 Stage 1: One-hot CE routing — no CLIP similarity file required.
# Lightweight — no SDXL needed. V100 16GB is sufficient.
#
# Key change from v2.0: --target_mode ce (one-hot cross-entropy on style label)
# instead of KL divergence against CLIP-similarity soft targets.
#
# Prerequisites:
#   1. wikiart_label_map.json must exist (run slurm/precompute_v2.sh first;
#      only the label map is needed — clip_similarity.pt is NOT required).
#
# Usage:
#   sbatch slurm/train_stage1_v21.sh
#
# Override examples:
#   MAX_STEPS=20000 sbatch slurm/train_stage1_v21.sh
#   RESUME=/scratch/eyavuz21/lora_attention/stage1_v21/latest.pt sbatch slurm/train_stage1_v21.sh
#

#SBATCH --job-name=MoELoRA-S1v21
#SBATCH --nodes=1
...
echo "----------------------------------------"
echo "target_mode: ce (v2.1 — one-hot cross-entropy)"
