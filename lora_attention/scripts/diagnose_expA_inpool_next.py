#!/usr/bin/env python3
"""Focused expA_inpool diagnostic: base vs reference B-LoRA vs synth variants."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output_dir", default="/scratch/eyavuz21/lora_attention/diagnostics/expA_inpool_next")
    ...
    p.add_argument("--prompt", default="A detailed painting")
...
def main() -> None:
    ...
    print(f"[diag] output_dir={out_root}")
    ...
    model.encoder.eval()
    print(f"[diag] loaded checkpoint step={ckpt.get('step', '?')}")
    ...
    for case in CASES:
        ...
        with torch.no_grad():
            A_top1, synth_top1 = model.forward(
                q,
                pool_indices,
                temperature=args.temperature,
                top_k=1,
        ...
        entropy = model.attention_entropy(A_top1).item()
        print(f"[diag] gt_rank={gt_rank} entropy={entropy:.4f}/{math.log(len(pool_indices)):.4f}")
        print("[diag] top5=" + ", ".join(f"{n}:{v.item():.4f}" for n, v in zip(top_names, top_vals)))

        base = generate(pipe, args.prompt, args.seed, args.num_inference_steps, args.guidance_scale)
        ...
        ref = generate(pipe, args.prompt, args.seed, args.num_inference_steps, args.guidance_scale)
        ref.save(case_dir / "reference_blora.png")
        unload_lora(pipe)

        synth_cpu = {k: v.detach().cpu() for k, v in synth_top1.items()}
        inject_lora(pipe, synth_cpu, style_alpha=args.style_alpha)
        synth = generate(pipe, args.prompt, args.seed, args.num_inference_steps, args.guidance_scale)
        synth.save(case_dir / "synth_top1_alpha1.png")
        unload_lora(pipe)
        ...
        synth_nm = generate(pipe, args.prompt, args.seed, args.num_inference_steps, args.guidance_scale)
        synth_nm.save(case_dir / "synth_top1_norm_match.png")
        unload_lora(pipe)

        query_copy = style_image.resize((320, 320), Image.LANCZOS)
        image_grid(
            [
                ("query", query_copy),
                ("base", base),
        ...
        summary["cases"].append(
            {
                ...
                "grid": str(case_dir / "comparison_grid.png"),
            }
        )

    (out_root / "summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(f"\n[diag] complete: {out_root}")
    print(f"[diag] summary: {out_root / 'summary.json'}")
